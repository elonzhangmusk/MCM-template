...
if __name__ == "__main__":
    α, φ, y1, y2, H, B1, B2, b1, b2, L, t, d1, u, bt, btp, bt1 = sp.symbols(
        'α φ y1 y2 H B1 B2 b1 b2 L t d1 u bt btp bt1'
    )

    # 定义原函数
    C = y1 * (H * B1 * t / 2 + (B1 + B2 + b2) * d1 * (4 * L + 3 * t) + 1 / 2 * (b1 + b2) * (4 * L + 3 * t) * H)
    EA = 1 / 2 * y2 * H ** 2 * (4 * L + 3 * t) * (sp.cos(φ - btp)) ** 2 / (
            (sp.cos(btp)) ** 2 * sp.cos(btp + φ / 3) * (1 + sp.sqrt(
        (sp.sin(4* φ / 3 ) * sp.sin(φ - bt1)) / (sp.cos(btp + φ / 3) * sp.cos(btp - bt1)))) ** 2)
    ...
    function1 = (u * (C + EA * sp.sin(bt - sp.pi / 2) + EB * sp.sin(α - sp.pi / 2)) + EB * sp.cos(α - sp.pi / 2)) / (EA * sp.cos(bt - sp.pi / 2))
    GG = 3 * y1 * B1 * H * t / 2 * ((2 * (- H / sp.tan(bt)) + B1) / 3 + b1 - H / sp.tan(α) + B2) + y1 * (
            B1 + B2 + b2) * (4 * L + 3 * t) * d1 * (
                 B1 + B2 + b2) / 2 + (
                 (2 * b1 + b2) * (- H / sp.tan(α)) + (b1 + 2 * b2) * (- H / sp.tan(α) + b1) / 3 / (
                 b1 + b2) + B2) * H * (
                 b1 + b2) * (4 * L + 3 * t) / 2
    function2 = (GG + (B2 + b2 + H / (3 * sp.tan(bt))) * EA * sp.sin(bt - sp.pi / 2) + (
            B2 - H / (3 * sp.tan(α))) * EB * sp.sin(α - sp.pi / 2)) / (
                        (EA * sp.cos(bt - sp.pi / 2) + EB * sp.cos(α - sp.pi / 2)) * H / 3)
    ...
    # 设置参数值和变量范围
    param_values = {
        y1: 24.0, y2: 18.0, H: 6.0, B1: 2.0, B2: 1.0,
        b1: 0.3, b2: 2.5, L: 2.5, t: 0.4,
        d1: 0.3, u: 0.6, bt: sp.pi / 2, btp: 0,
        bt1: 0
    }
    hk = 0.001
    var_ranges = {
        'α': (np.pi / 2, np.pi / 18 * 17, 150),
        'φ': (0 + hk, np.pi / 18 * 8, 150)
    }
    ...
    expanded_var_ranges = {
        ...
        'φ': (-np.pi / 4, np.pi / 2, 200)  # 扩大fy范围并包含负值
    }
